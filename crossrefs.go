...
func (this *PdfParser) lookupObjectViaOS(sobjNumber int, objNum int) (PdfObject, error) {
	...
	if !cached {
		...
		if err != nil {
			Log.Debug("Missing object stream with number %d", sobjNumber)
			return nil, err
		}
		...
		if !ok {
			Log.Debug("ERROR: Object stream should always have a Type")
			return nil, errors.New("Object stream missing Type")
		}
		if strings.ToLower(string(*name)) != "objstm" {
			Log.Debug("ERROR: Object stream type shall always be ObjStm !")
	...
	if err != nil {
		Log.Debug("ERROR Fail to read object (%s)", err)
		return nil, err
	}
	if val == nil {
...
func (this *PdfParser) lookupByNumber(objNumber int, attemptRepairs bool) (PdfObject, bool, error) {
	...
	if xref.xtype == XREF_TABLE_ENTRY {
		...
		if err != nil {
			Log.Debug("ERROR Failed reading xref (%s)", err)
			// Offset pointing to a non-object.  Try to repair the file.
			if attemptRepairs {
				Log.Debug("Attempting to repair xrefs (top down)")
				xrefTable, err := this.repairRebuildXrefsTopDown()
				if err != nil {
					Log.Debug("ERROR Failed repair (%s)", err)
		...
		if xref.osObjNumber == objNumber {
			Log.Debug("ERROR Circular reference!?!")
		...
		if exists {
			...
			if err != nil {
				Log.Debug("ERROR Returning ERR (%s)", err)
				return nil, true, err
			}
