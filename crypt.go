...
func (this *PdfCrypt) LoadCryptFilters(ed *PdfObjectDictionary) error {
	...
	for name, v := range *cf {
		...
		if name == "Identity" {
			Log.Debug("ERROR - Cannot overwrite the identity filter - Trying next")
			continue
		}
...
func PdfCryptMakeNew(ed, trailer *PdfObjectDictionary) (PdfCrypt, error) {
	...
	if !ok {
		Log.Debug("ERROR Crypt dictionary missing required Filter field!")
		return crypter, errors.New("Required crypt field Filter missing")
	}
	if *filter != "Standard" {
		Log.Debug("ERROR Unsupported filter (%s)", *filter)
	...
	if L, ok := (*ed)["Length"].(*PdfObjectInteger); ok {
		if (*L % 8) != 0 {
			Log.Debug("ERROR Invalid encryption length")
	...
	if ok {
		if *V >= 1 && *V <= 2 {
			...
			Log.Debug("ERROR Unsupported encryption algo V = %d", *V)
	...
	if len(*U) != 32 {
		...
		Log.Debug("Warning: Length(U) != 32 (%d)", len(*U))
...
func (this *PdfCrypt) makeKey(filter string, objNum, genNum uint32, ekey []byte) ([]byte, error) {
	...
	if !ok {
		Log.Debug("ERROR Unsupported crypt filter (%s)", filter)
...
func (this *PdfCrypt) decryptBytes(buf []byte, filter string, okey []byte) ([]byte, error) {
	...
	if cfMethod == "V2" {
		...
		if len(buf) < 16 {
			Log.Debug("ERROR AES invalid buf %s", buf)
