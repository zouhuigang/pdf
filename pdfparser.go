	...
	for {
		...
		if !nameStarted {
			...
			if bb[0] == '/' {
				...
				Log.Debug("ERROR Name starting with %s (% x)", bb, bb)
...
func parseReference(refStr string) (PdfObjectReference, error) {
	...
	if len(result) < 3 {
		Log.Debug("Error parsing reference")
...
func (this *PdfParser) parseObject() (PdfObject, error) {
	...
	for {
		...
		if bb[0] == '/' {
			...
			Log.Debug("ERROR Unknown (peek \"%s\")", peekStr)
...
func (this *PdfParser) parseDict() (*PdfObjectDictionary, error) {
	...
	for {
		...
		if err != nil {
			Log.Debug("ERROR Returning name err %s", err)
			return nil, err
		}
...
func (this *PdfParser) parsePdfVersion() (float64, error) {
	...
	if len(result1) < 2 {
		Log.Debug("Error: PDF Version not found!")
	...
	for {
		...
		if len(result2) == 4 {
			if insideSubsection == false {
				Log.Debug("ERROR Xref invalid format!\n")
		...
		if (len(txt) > 6) && (txt[:7] == "trailer") {
			...
			if err != nil {
				Log.Debug("Error parsing trailer dict (%s)", err)
				return nil, err
			}
			break
		}
		...
		if txt == "%%EOF" {
			Log.Debug("ERROR: end of file - trailer not found - error!")
...
func (this *PdfParser) parseXrefStream(xstm *PdfObjectInteger) (*PdfObjectDictionary, error) {
	...
	if err != nil {
		Log.Debug("ERROR: Failed to read xref object")
	...
	if !ok {
		Log.Debug("ERROR: XRefStm pointing to non-stream object!")
	...
	if !ok {
		Log.Debug("ERROR: Missing size from xref stm")
	...
	wLen := len(*wArr)
	if wLen != 3 {
		Log.Debug("ERROR: Unsupported xref stm (len(W) != 3 - %d)", wLen)
	...
	if err != nil {
		Log.Debug("ERROR: Unable to decode stream")
		return nil, err
	}
	...
	if entries != len(indexList) {
		...
		Log.Debug("ERROR: xref stm: num entries != len(indices) (%d != %d)", entries, len(indexList))
	...
	for i := 0; i < len(ds); i += deltab {
		...
		if ftype == 0 {
			...
			Log.Debug("ERROR: --------INVALID TYPE XrefStm invalid?-------")
...
func (this *PdfParser) parseXref() (*PdfObjectDictionary, error) {
	...
	if reIndirectObject.MatchString(string(bb)) {
		...
		Log.Debug("ERROR: Invalid xref.... starting with \"%s\"", string(bb))
...
func (this *PdfParser) loadXrefs() (*PdfObjectDictionary, error) {
	...
	if ind == nil {
		Log.Debug("Error: EOF marker not found!")
	...
	if offsetXref > fSize {
		Log.Debug("ERROR: Xref offset outside of file")
		Log.Debug("Attempting repair")
		offsetXref, err = this.repairLocateXref()
		if err != nil {
			Log.Debug("ERROR: Repair attempt failed (%s)")
			return nil, err
		}
	}
	// Read the xref.
	...
	for present {
		...
		if err != nil {
			Log.Debug("ERROR: Failed loading another (Prev) trailer")
			return nil, err
		}
		...
		if present {
			...
			if intInSlice(int64(prevoff), prevList) {
				...
				Log.Debug("Preventing circular xref referencing")
				break
			}
...
func (this *PdfParser) parseIndirectObject() (PdfObject, error) {
	...
	if err != nil {
		Log.Debug("ERROR: Fail to read indirect obj")
		return &indirect, err
	}
	...
	if len(indices) < 6 {
		Log.Debug("ERROR: Unable to find object signature (%s)", string(bb))
	...
	if err != nil {
		Log.Debug("ERROR: unable to read - %s", err)
		return nil, err
	}
	...
	if len(result) < 3 {
		Log.Debug("ERROR: Unable to find object signature (%s)", string(hb))
		return &indirect, errors.New("Unable to detect indirect object signature")
	}
		...
		if isWhiteSpace(bb[0]) {
			...
			if bb[0] == 'e' {
				...
				if string(bb[:6]) == "stream" {
					...
					if err != nil {
						Log.Debug("ERROR stream (%d): %X", len(stream), stream)
						return nil, err
					}
...
func NewParser(rs io.ReadSeeker) (*PdfParser, error) {
	...
	if err != nil {
		Log.Debug("ERROR: Failed to load xref table! %s", err)
