...
func (this *PdfParser) decodeStream(obj *PdfObjectStream) ([]byte, error) {
	...
	if *method == "FlateDecode" {
		...
		if hasDecodeParams {
			...
			if hasbits {
				...
				if !ok {
					Log.Debug("ERROR: Invalid BitsPerComponent")
		...
		if hasDecodeParams && predictor != 1 {
			if predictor == 2 { // TIFF encoding: Needs some tests.
				...
				if !ok {
					Log.Debug("ERROR: Predictor Column missing\n")
				...
				if len(outData)%rowLength != 0 {
					Log.Debug("ERROR: TIFF encoding: Invalid row length...")
				...
				rows := len(outData) / rowLength
				if len(outData)%rowLength != 0 {
					Log.Debug("ERROR: Invalid row length...")
					return nil, fmt.Errorf("Invalid row length (%d/%d)", len(outData), rowLength)
				}
				...
				for i := 0; i < rows; i++ {
					...
					default:
						Log.Debug("ERROR: Invalid filter byte (%d)", fb)
				...
				Log.Debug("ERROR: Unsupported predictor (%d)", predictor)
		...
		for {
			...
			if (b >= 'a' && b <= 'f') || (b >= 'A' && b <= 'F') || (b >= '0' && b <= '9') {
				...
				Log.Debug("ERROR: Invalid ascii hex character (%c)", b)
	...
	Log.Debug("ERROR: Unsupported encoding method!")
