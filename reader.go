...
func (this *PdfReader) Decrypt(password []byte) (bool, error) {
	...
	if err != nil {
		Log.Debug("ERROR: Fail to load structure (%s)", err)
		return false, err
	}
...
func (this *PdfReader) loadStructure() error {
	...
	if err != nil {
		Log.Debug("ERROR: Failed to read root element catalog: %s", err)
		return err
	}
	pcatalog, ok := oc.(*PdfIndirectObject)
	if !ok {
		Log.Debug("ERROR: Missing catalog: (root %q) (trailer %s)", oc, *(this.parser.trailer))
		return errors.New("Missing catalog")
	}
	catalog, ok := (*pcatalog).PdfObject.(*PdfObjectDictionary)
	if !ok {
		Log.Debug("ERROR: Invalid catalog (%s)", pcatalog.PdfObject)
	...
	if err != nil {
		Log.Debug("ERROR: Failed to read pages")
		return err
	}
	ppages, ok := op.(*PdfIndirectObject)
	if !ok {
		Log.Debug("ERROR: Pages object invalid")
		Log.Debug("op: %p", ppages)
		return errors.New("Pages object invalid")
	}
	pages, ok := ppages.PdfObject.(*PdfObjectDictionary)
	if !ok {
		Log.Debug("ERROR: Pages object invalid (%s)", ppages)
		return errors.New("Pages object invalid")
	}
	pageCount, ok := (*pages)["Count"].(*PdfObjectInteger)
	if !ok {
		Log.Debug("ERROR: Pages count object invalid")
	...
	if err != nil {
		Log.Debug("ERROR: Failed to build outline tree (%s)", err)
		return err
	}
...
func (this *PdfReader) loadOutlines() (*PdfOutlineTreeNode, error) {
	...
	if err != nil {
		Log.Debug("ERROR: Failed to read outlines")
		return nil, err
	}
	Log.Debug("Outline root: %v", outlineRootObj)

	if _, isNull := outlineRootObj.(*PdfObjectNull); isNull {
		Log.Debug("Outline root is null - no outlines")
		return nil, nil
	}
	...
	Log.Debug("Outline root dict: %v", dict)

	outlineTree, err := this.buildOutlineTree(dict)
	if err != nil {
		return nil, err
	}
	Log.Debug("Resulting outline tree: %v", outlineTree)
...
func (this *PdfReader) buildOutlineTree(obj PdfObject) (*PdfOutlineTreeNode, error) {
	...
	Log.Debug("build outline tree: dict: %v", dict)
...
func (this *PdfReader) GetOutlinesFlattened() ([]*PdfOutlineTreeNode, []string, error) {
	...
	flattenFunc = func(node *PdfOutlineTreeNode, outlineList *[]*PdfOutlineTreeNode, titleList *[]string, depth int) {
		...
		if node.context == nil {
			Log.Debug("ERROR: Missing node.context") // Should not happen ever.
			return
		}
...
func (this *PdfReader) GetForms() (*PdfObjectDictionary, error) {
	...
	if dict, hasFormsDict := (*catalog)["AcroForm"].(*PdfObjectDictionary); hasFormsDict {
		...
		if err != nil {
			Log.Debug("ERROR: Failed to read forms")
			return nil, err
		}
	...
	if err != nil {
		Log.Debug("ERROR: Unable to traverse AcroForms (%s)", err)
		return nil, err
	}
...
func (this *PdfReader) buildPageList(node *PdfIndirectObject, parent *PdfIndirectObject) error {
	...
	Log.Debug("buildPageList node type: %s", *objType)
	if *objType == "Page" {
	...
	if *objType != "Pages" {
		Log.Debug("ERROR: Table of content containing non Page/Pages object! (%s)", objType)
	...
	if err != nil {
		Log.Debug("ERROR: Failed loading Kids object")
		return err
	}
	...
	for idx, child := range *kids {
		...
		if !ok {
			Log.Debug("ERROR: Page not indirect object - (%s)", child)
...
func (this *PdfReader) traverseObjectData(o PdfObject) error {
	...
	if _, isRef := o.(*PdfObjectReference); isRef {
		Log.Debug("ERROR: Reader tracing a reference!")
