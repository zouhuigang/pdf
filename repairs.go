...
func (this *PdfParser) repairLocateXref() (int64, error) {
	...
	if len(results) < 1 {
		Log.Debug("ERROR: Repair: xref not found!")
...
func (this *PdfParser) rebuildXrefTable() error {
	...
	for objNum, xref := range this.xrefs {
		...
		if err != nil {
			Log.Debug("ERROR: Unable to look up object (%s)", err)
			Log.Debug("ERROR: Xref table completely broken - attempting to repair ")
			xrefTable, err := this.repairRebuildXrefsTopDown()
			if err != nil {
				Log.Debug("ERROR: Failed xref rebuild repair (%s)", err)
				return err
			}
			this.xrefs = *xrefTable
			Log.Debug("Repaired xref table built")
			return nil
		}
...
func (this *PdfParser) repairRebuildXrefsTopDown() (*XrefTable, error) {
	...
	for {
		...
		if len(results) > 0 {
			...
			if err != nil {
				Log.Debug("ERROR: Unable to parse indirect object (%s)", err)
				return nil, err
			}
			...
			Log.Debug("Not an object - stop repair rebuilding xref here (%s)", peakBuf)
			break
		}
	}
